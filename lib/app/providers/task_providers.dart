...
class CompleteTaskNotifier extends Notifier<void> {
  ...
  Future<void> complete(Task task, {TaskStatus status = TaskStatus.completed}) async {
...
/// 时间线事件流（§9）：支持疾病 / 类型过滤。
final timelineEventsProvider = StreamProvider.family<List<HealthEvent>, TimelineFilter>(
  (ref, filter) {
    final repo = ref.watch(repositoryProvider);
